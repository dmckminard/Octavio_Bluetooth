...
def checkServer():
    ...
    if (("Error in socket shutdown" or "Exception in Controller" or "Started Snapcast client.") in lastLine) :
        ...
        GPIO.cleanup()
        subprocess.call(["python3", "/home/config/bin/scan_octavio.py", "|logger -t octavio"])
        GPIO.cleanup()
        sys.exit()
        return 0
...
def name():
    ...
    for client in server.clients:
        if client.identifier == macaddr :
            ...
            # HOSTNAME BLUETOOTH - ON NE LE FAIT PAS POUR EVITER UNE CONFUSION CLIENT
    ...
    for client in server.clients:
        ...
        # Check stream used by this Octavio
            ...
            if "Spotify" in actual_stream_line :
                ...
                colorWipe(strip, Color(0,51,0), 0) #GREEN FOR SPOTIFY
                time.sleep(0.5)
                colorWipe(strip, Color(0,0,0), 0)
            if "Jack" in actual_stream_line :
                print(' STREAM JACK')
                colorWipe(strip, Color(51,0,0), 0) #RED FOR JACK
                time.sleep(0.5)
                colorWipe(strip, Color(0,0,0), 0)
